...
def load_data():
    with open(os.path.join(cwd, "data", "irish.abc"), "r") as f:
        text = f.read()
    songs = extract_songsnippet(text)
    return songs

def extract_songsnippet(text):
    pattern = '(^|\n\n)(.*?)\n\n'
...
def play(song):
    ...
    if ret == 0: #did not suceed
        return playwav(basename+'.wav')
    return None

def play_generatedsong(generated_text):
    songs = extract_songsnippet(generated_text)
    if len(songs) == 0:
